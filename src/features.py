...
class Features:
    ...
    def spectral_features(self, data):
        ...
        spec_spread = np.sqrt(np.sum(((freqs-spec_centroid)**2)*magnitudes) / (sum_mag + 1e-12))

        #spectral skewness = distribution of the spectrum around its mean
        spec_skewness = np.sum(((freqs-spec_centroid)**3)*magnitudes) / ((spec_spread**3)*sum_mag + 1e-12)

        #spectral kurtosis = flatness of spectrum around its mean
        spec_kurtosis =  np.sum(((freqs-spec_centroid)**4)*magnitudes) / ((spec_spread**4)*sum_mag + 1e-12)
